//
//  AboutView.swift
//  iOS
//
//  Created by Venkat on 20/9/20.
//

import SwiftUI

struct AboutView: View {
    
    @Environment(\.dismiss) var dismiss
    
    var body: some View {
        NavigationStack {
            List {
                AboutView.appVersionSection()
                AboutView.TipsSection()
                AboutView.FeedbackSection()
                AboutView.HowItWorksSection()
                AboutView.CreditsSection()
                AboutView.PrivacySection()
                
            }
            .listStyle(InsetGroupedListStyle())
            .navigationBarTitleDisplayMode(.inline)
            .navigationTitle("About")
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(role: .close) {
                        dismiss()
                    }
                }
            }
        }
        
    }
    // Adds section with app version and build number
    private static func appVersionSection() -> some View {
        Section(header: Text(appVersion())) {
            Text("Made, with ❤️, by [Venkat](https://venkatasg.net). Inspired by Detexify, I wanted to make a native iOS app for translating hand-drawn symbols to their corresponding LaTeX commands that was fast, efficient, and lightweight.")
        }
    }
    
    // Adds section with app version and build number
    private static func TipsSection() -> some View {
        Section(header: Text("Tips")) {
            Text("💡You can long-press on a symbol to see a larger version, and get options to copy the LaTeX symbol, unicode codepoint, or symbol directly!")
            Text("⚡Double tap the symbol or LaTeX command to copy them to the clipboard instantly.")
        }
    }
    
    // Options for sending feedback
    private static func FeedbackSection() -> some View {
        Section(header:Text("Feedback")) {
            Text("You can [email me](mailto:venkat@venkatasg.net) for support, to report bugs, or suggest new features.")
            HStack {
                Image(systemName: "heart.circle.fill")
                    .foregroundColor(.pink)
                    .accessibility(label: Text("Please Rate & Review DeTeXt"))
                Link("Rate and Review on App Store", destination: URL(string: "itms-apps://itunes.apple.com/app/id1531906207?action=write-review")!)
                Spacer()
                Image(systemName: "chevron.forward")
                    .foregroundColor(.blue)
            }
        }
    }
    
    // Details on how the app works
    private static func HowItWorksSection() -> some View {
        Section(header: Text("How it works")) {
            Text("DeTeXt uses a [mobilenet_v2](https://pytorch.org/hub/pytorch_vision_mobilenet_v2/) PyTorch model trained on the Detexify training data, which was then translated to CoreML using [coremltools](https://coremltools.readme.io).\n\nThe symbol images are generated by converting PDF files containing each symbol (made with LaTeX) to SVG files.\n\nThe source code for DeTeXt is available [on GitHub](https://github.com/venkatasg/DeTeXt) under the MIT License.\n\nUnicode codepoints for many of the symbols were obtained from the source repository of [Unicodeit](https://github.com/svenkreiss/unicodeit) and [this translation dictionary from a GitHub gist](https://gist.github.com/Metaxal/86be1b733c0f5ad4a0cf6c58cf140436#file-latex-math-chars-rkt). I manually added the codepoints for several symbols, and will continue adding missing codepoints as I update the app.")
                .font(.body)
            
        }
    }
    
    // Credits Section
    private static func CreditsSection() -> some View {
        Section(header: Text("Credits")) {
            Text("Thanks to Daniel Kirsch and the team at [Detexify](http://detexify.kirelabs.org) for their cool open-source web app that inspired me to make DeTeXt, and for providing the training data and prompt responses to my questions.\n\nThanks to [Will Bishop](https://willbish.com) for helping me figure out a tricky problem I had with the PencilKit APIs.\n\nA big thanks to [Hans](https://twitter.com/SherlockHans) for beta testing new features and giving valuable feedback.\n\nThanks to [Adnan Haddad](https://cheesecakeco.de) for suggesting the Unicode character/codepoint copy feature, and pushing me towards implementing it.\n\nFinally a big thanks to all the folks on the [NetNewsWire](https://ranchero.com/netnewswire/) Slack for being supportive and inspiring me to build my own iOS app.")
        }
    }
    
    // Privacy Section
    private static func PrivacySection() -> some View {
        Section(header: Text("Privacy")) {
            Text("DeTeXt does not collect or store any personal data or information. All processing of drawings to find the corresponding symbol happens on your device.\n\nDeTeXt does not display any advertisements, use any trackers or analytics, or send any data to any server.")
        }
    }
}


struct AboutView_Previews: PreviewProvider {

    static var previews: some View {
        AboutView()
    }
}
